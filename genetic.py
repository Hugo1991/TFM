...
class graph_operations:
    ...
    def computeClusters(self):
        ...
        self.initHVS()
        self.generateHVSs()
        # Unir los HVS que presenten una conectividad interna menor que la que tienen entre sí.
        ...
        while (self.counter < self.limit and not completed):                        
            if (self.completed_evolution(fitness_values)):
                ...
                if self.counter % 10 == 0:
                    ...
                    print("La precisión en la generación",self.counter,"es de",max(fitness_values),"sobre",self.target)
                    result = ""
        ...
        if self.prob_mutation > mutate:
            ...
            for _ in range(0,margin):
                ...
                if self.artificial_mutation == True:
                    ...
                    # Arreglo para trampear la mutación y obtener la solución.
        ...
        while (self.counter < self.limit and not completed):                        
            if (self.completed_evolution(fitness_values)):
                ...
                if self.counter % 10 == 0:
                    ...
                    print("La precisión en la generación",self.counter,"es de",max_value,"sobre",self.target)
                    result = ""
                    for i in range(0,len(parent1)):
                        result = result + parent1[i].__str__() + " "
                    print(result)
